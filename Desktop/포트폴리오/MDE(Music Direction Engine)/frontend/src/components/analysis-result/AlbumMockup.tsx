...
interface AlbumMockupProps {
  ...
  imageUrl?: string | null   // pre-generated URL from backend
}
    ...
    <div style={{
      ...
      <div style={{
        ...
        <span style={{
          ...
          borderRadius: '6px', background: 'rgba(37,99,235,0.07)',
      ...
      <div style={{ padding: '24px' }}>
        ...
        {loadState === 'idle' && (
          <div style={{ display: 'flex', gap: '20px', alignItems: 'center', flexWrap: 'wrap' }}>
            {/* 미리보기 플레이스홀더 */}
            <div style={{
              width: '120px', height: '120px', borderRadius: '12px', flexShrink: 0,
              ...
              <span style={{ fontSize: '10px', color: colors.from, fontWeight: 600, opacity: 0.7 }}>
                {firstGenre || 'cover'}
              </span>
            </div>
            <div style={{ flex: 1, minWidth: '180px' }}>
              <p style={{ fontSize: '14px', fontWeight: 700, color: '#0f0f14', marginBottom: '6px' }}>
                앨범 커버 목업 생성
              </p>
              <p style={{ fontSize: '12px', color: '#6b6b8a', lineHeight: 1.6, marginBottom: '14px' }}>
                {profile.visual_association?.slice(0, 3).join(', ')} 등의 비주얼 연상어를 바탕으로
                Flux AI가 이미지를 생성합니다.
              </p>
              <button
                onClick={generate}
                style={{
                  ...
                  boxShadow: `0 4px 14px -2px ${colors.from}55`,
                }}
              >
                목업 생성하기
              </button>
        ...
        {/* ── loading / done (single img element) ── */}
        {(loadState === 'loading' || loadState === 'done') && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={{
              maxWidth: '280px', width: '100%', margin: '0 auto',
              position: 'relative',
            }}>
              {/* 이미지 컨테이너 */}
              <div style={{
                aspectRatio: '1', width: '100%', borderRadius: '14px',
                overflow: 'hidden', position: 'relative',
                background: `linear-gradient(135deg, ${colors.from}18, ${colors.to}18)`,
                border: loadState === 'done' ? '1px solid #e8e8f0' : `1px solid ${colors.from}33`,
                boxShadow: loadState === 'done' ? '0 8px 30px rgba(0,0,0,0.1)' : 'none',
                transition: 'box-shadow 0.4s ease',
              }}>
                {/* 실제 이미지 - 항상 렌더링 (opacity로 전환) */}
                ...
                {loadState === 'loading' && (
                  <div style={{
                    ...
                    <div style={{
                      ...
                      backgroundImage: `radial-gradient(${colors.from}40 1px, transparent 1px)`,
                      backgroundSize: '18px 18px',
                    }} />
                    <div style={{
                      width: '44px', height: '44px', borderRadius: '50%',
                      border: `3px solid ${colors.from}30`,
                      borderTopColor: colors.from,
                    ...
                    <div style={{ textAlign: 'center', position: 'relative' }}>
                      ...
                      <p style={{ fontSize: '11px', color: '#9898b0', marginTop: '2px' }}>
                        {elapsed}초 · 보통 10-30초 소요
                      </p>
                    </div>
                  </div>
                )}
            ...
            {loadState === 'done' && (
              <>
                ...
                <div style={{
                  ...
                  background: '#f9f9fb', border: '1px solid #ebebf0',
                }}>
                  <p style={{ fontSize: '10px', color: '#b0b0c8', fontWeight: 600, marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                    생성 프롬프트
                  </p>
                  <p style={{
                    fontSize: '11px', color: '#6b6b8a', lineHeight: 1.6,
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button
                    ...
                    style={{
                      ...
                      fontSize: '12px', fontWeight: 600, color: '#6b6b8a',
                      cursor: 'pointer',
                    }}
                  >
                    다시 생성
                  ...
                  <a
                    ...
                    style={{
                      ...
                      fontSize: '12px', fontWeight: 600, color: colors.from,
            ...
            {loadState === 'loading' && (
              <p style={{ fontSize: '11px', color: '#b0b0c8', textAlign: 'center' }}>
                Pollinations.ai Flux 모델로 생성 중입니다
              </p>
            )}
          </div>
        )}
        ...
        {loadState === 'error' && (
          <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <p style={{ fontSize: '12px', color: '#9898b0', marginBottom: '12px' }}>
                Pollinations.ai 서버가 혼잡합니다. 잠시 후 다시 시도해보세요.
              </p>
              <button
                onClick={generate}
                style={{
                  ...
                  background: '#f5f5f8', border: '1px solid #e8e8f0',
                  fontSize: '12px', fontWeight: 600, color: '#6b6b8a', cursor: 'pointer',
                }}
              >
                다시 시도
              </button>
