<img width="1439" height="726" alt="분석히스토리1" src="https://github.com/user-attachments/assets/9f2fdc08-5fd5-4ebe-8d2a-045596e87512" /><img width="1439" height="726" alt="분석히스토리2" src="https://github.com/user-attachments/assets/0aeafc78-fa4c-45c0-8f8e-4e44f6af3b09" /># MDE — Music Direction Engine
...
MusicProfile(객체 속성 10개)과 DirectionExplanation(설명 텍스트 4개)을 하나의 프롬프트에서 동시에 생성하면 두 가지 문제가 생깁니다.

첫째, 프롬프트 복잡도가 올라갈수록 LLM이 스키마를 이탈할 확률이 높아집니다. 특히 무료 모델처럼 파라미터가 상대적으로 작은 모델은 복잡한 중첩 스키마를 한 번에 정확히 생성하기 어렵습니다.

둘째, 두 객체는 생성 특성이 다릅니다. MusicProfile은 분류 레이블과 수치형 데이터 중심이고, DirectionExplanation은 서술형 텍스트 중심입니다. 이를 분리하면 각 호출에 최적화된 프롬프트를 구성할 수 있고, 실패 시 해당 단계만 재시도할 수 있습니다.

asyncio.gather()로 두 호출을 병렬 실행하기 때문에 직렬 실행 대비 전체 지연 시간은 단일 호출 수준으로 유지됩니다.
...
MusicProfile과 DirectionExplanation은 서로 의존하지 않습니다. 이 두 생성 작업을 `asyncio.gather()`로 병렬 실행하면 직렬 실행 대비 응답 시간을 절반으로 줄일 수 있습니다.
...
현재 gpt-oss-20b 기준 평균 응답 시간은 약 20–25초입니다. 직렬 실행 시 40–50초가 소요될 수 있습니다.
...
앨범 커버 목업 이미지는 Pollinations.ai의 Flux 모델을 통해 생성됩니다. 이미지 생성 요청은 URL 파라미터 방식으로 이루어지며, 백엔드는 이미지 URL만 생성하고 실제 이미지 데이터는 프론트엔드에서 직접 fetch합니다.

이 설계는 백엔드의 메모리와 대역폭을 절약합니다. 이미지 바이너리가 백엔드를 경유하지 않기 때문입니다.
...
OpenRouter를 통해 접근 가능한 무료 instruction 모델입니다. reasoning 모델(nemotron, minimax 등)은 응답 시간이 60–90초에 달하고 추론 텍스트가 JSON 앞에 출력되어 파싱이 까다롭습니다. gpt-oss-20b는 instruction following이 안정적이고 평균 응답 시간이 20–25초로 허용 가능한 범위 내에 있습니다.

`extract_json()` 파싱 로직은 gpt-oss-20b뿐 아니라 reasoning 모델도 처리할 수 있도록 설계되어 있어 모델 교체에 유연합니다.
...
처음에는 추론 능력이 높은 reasoning 모델을 선택했습니다. 결과 품질은 좋았지만 응답 시간이 60–90초에 달했습니다. 이는 사용자가 허용 가능한 범위를 벗어납니다. instruction following 능력은 충분하지만 빠른 모델로 전환하면서 응답 시간을 20–25초로 줄였습니다. 모델 성능과 응답 시간의 트레이드오프는 실제 서비스에서 항상 고려해야 하는 요소입니다.
